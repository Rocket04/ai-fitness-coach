 ...
 * @param {Array<Object>} checkins — все чек-ины
 * @param {Array<Object>} allCheckins — полный массив (для расчёта recoveryScore)
 * @param {number} days — глубина в днях (7 или 30)
 * @returns {Array<{date:string, recoveryScore:number, hrv:number, restHR:number, sleepHours:number}>}
 */
export function getTrendData(checkins, allCheckins, days = 30) {
 ...
 * @param {Array<Object>} sessions — все сессии
 * @param {number} days — глубина
 * @returns {Array<{date:string, rpe:number, type:string}>}
 */
export function getRpeTrend(sessions, days = 30) {
  const today = new Date();
  const startDate = addDays(today, -(days - 1));
  const startStr = formatISO(startDate);
 ...
 * @param {Array} trendData — результат getTrendData
 * @returns {Array<{type:string, severity:string, metric:string, consecutiveDays:number,
 *   currentValue:number, message:string, recommendation:string, apreAction:string}>}
 */
export function detectNegativeTrends(trendData) {
  const warnings = [];
 ...
 * @param {Array} trendData — результат getTrendData
 * @returns {Array<{weekStart:string, avgRecoveryScore:number, avgHrv:number, avgRestHR:number}>}
 */
export function getWeeklyAverages(trendData) {
 ...
 * @param {Array} trendData — результат getTrendData
 * @param {Array} weeklyAverages — результат getWeeklyAverages
 * @param {Object} weeklySummary — результат getWeeklySummary из engine
 * @returns {Object|null} { severity, title, message, recommendation, apreOverride } или null
 */
export function getOvertrainingWarning(trendData, weeklyAverages, weeklySummary) {
