// js/domains/recovery/advice.ts
// Советы тренера и объяснения авторегуляции (APRE)

import type { Checkin, Session, ReadinessStatus, SessionMode, TrendPoint, TestResults, WeeklySummary } from '../../shared/types.js';

/**
 * Генерирует советы тренера на основе Recovery Score и текущего состояния.
 */
export function getCoachAdvice(recoveryScore: number, checkin: Partial<Checkin> = {}, _testHistory: TestResults[] = [], weeklySummary: Partial<WeeklySummary> = {}): string[] {
  const advice = [];
