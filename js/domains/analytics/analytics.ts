...
import type { Checkin, Session, TrendPoint, TrendWarning, WeeklyAverage, WeeklySummary, OvertrainingWarning } from '../../shared/types.js';
...
export function calculateWeeklyCompletionRate(sessions: Session[], weekStart: Date): number {
  if (!sessions.length) return 0;

  const weekEnd = new Date(weekStart);
  weekEnd.setDate(weekEnd.getDate() + 7);
  const startStr = formatISO(weekStart);
  const endStr = formatISO(weekEnd);
  ...
  for (const session of weekSessions) {
    ...
    for (const er of session.exerciseResults) {
      totalPlanned += er.plannedSets;
      totalCompleted += er.completedSets;
    }
  }

  if (totalPlanned === 0) return 0;
  return Math.min(1, Math.max(0, totalCompleted / totalPlanned));
}
