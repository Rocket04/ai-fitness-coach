 ...
 * @param {boolean} [params.usesWeight=false] — калистеника с весом (кг)
 ...
 * }}
 */
export function calcApreSets({ protocol, currentRM, unit = 'kg', isCalisthenics = false, usesWeight = false, set3Reps = null, recoveryScore = 100 }) {
  ...
  if (isCalisthenics) {
    ...
    // When usesWeight: display actual kg, otherwise map to level
    const set2Reps = protocolDef[75].reps;

    if (usesWeight) {
      return {
        set1: { weight: currentRM, reps: protocolDef[50].reps, readonly: true },
        set2: { weight: currentRM, reps: set2Reps, readonly: true },
        set3: { weight: currentRM, reps: 'AMRAP', readonly: false },
        set4: {
          weight: set4Weight !== null ? Math.max(0, set4Weight) : null,
          reps: 'AMRAP',
        ...
        effectiveRM: currentRM,
        recoveryReduction: 0,
      };
    }

    // Legacy: map weight to level (1-5) for backward compatibility
 ...
 * если они доступны.
 * Принимает массив exerciseConfigs для подстановки пользовательских настроек
 * (currentRM, usesWeight) для weight-based калистеники.
 *
 ...
 * @param {Array<{ id: string, name: string, isCalisthenics: boolean, currentRM: number|null, usesWeight?: boolean }>} [exerciseConfigs=[]]
 * @returns {Array<import('../../../core/types.js').Exercise>}
 */
export function annotateExercisesWithApre(exercises, previousApreResults = [], exerciseConfigs = []) {
  if (!exercises) return [];
  ...
  return exercises.map(ex => {
    ...
    // Ищем пользовательскую конфигурацию для weight-based калистеники
    const userConfig = exerciseConfigs.find(c =>
      c.name === ex.n || ex.n?.toLowerCase().includes(c.name.toLowerCase())
    );

    const protocol = inferApreProtocol(ex.n, ex.r);
    const defaultAddedWeight = 0;
    const usesWeight = userConfig?.usesWeight ?? false;
    const configuredRM = userConfig?.currentRM ?? null;
    ...
    return {
      ...
      unit: 'kg',
      usesWeight,
      currentRM: usesWeight
        ? (prev ? prev.nextRM : (configuredRM ?? defaultAddedWeight))
        : (prev ? prev.nextRM : defaultAddedWeight),
      calisthenicLevel: usesWeight ? undefined : (prev ? (prev.calisthenicLevel ?? defaultAddedWeight) : defaultAddedWeight),
    };
  });
}
